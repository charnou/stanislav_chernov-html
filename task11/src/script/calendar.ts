window.addEventListener('load', () => {
                ...
                (i == 2) ? week = 'We' :
                    (i == 3) ? week = 'Th' :
                        (i == 4) ? week = 'Fr' :
                            (i == 5) ? week = 'Sa' :
                                week = 'Su';
        let w: HTMLElement = document.createElement('div');
        calendarWeek.appendChild(w).innerHTML = week;
    }

    const calendarGrid: HTMLElement = document.createElement('div');
    calendarGrid.className = 'calendar__grid';
    calendar.appendChild(calendarGrid);

    let time: time = new Date().getTime();

    let monthToChange: boolean = true;
    ...
    function monthOrYearIsChanging(): void {
        if (monthToChange) {
    ...
    function isLeapYear(): boolean {
        let isLeapYear: boolean | number = new Date(time).getFullYear();
        if (isLeapYear % 4 == 0) {
            if (isLeapYear % 100 == 0) {
                if (isLeapYear % 400 == 0) {
                    isLeapYear = true;
    ...
    function monthToString(month: number): monthName {
    ...
    function updateDate(): void {
        let updateMonth: monthName = monthToString(new Date(time).getMonth());
        calendarMonth.innerHTML = updateMonth;
        let updateYear: number = new Date(time).getFullYear();
        calendarYear.innerHTML = updateYear.toString();
    }

    function prevDates(): void {
        let prevDates: number = new Date(time).setMonth(new Date(time).getMonth() - 1);
        prevDates = new Date(prevDates).setDate(daysInMonth(new Date(prevDates).getMonth()))
        let length: number = new Date(time).getDay();
        (length == 0) ? length = 6 : length--;
        for (; length != 0; length--) {
            let dates: HTMLElement = document.createElement('div');
            ...
            calendarGrid.insertBefore(dates, calendarGrid.firstChild).innerHTML = (new Date(prevDates).getDate()).toString();
            prevDates = new Date(prevDates).setDate(new Date(prevDates).getDate() - 1);
        }
    }

    function mainDates(): void {
        for (let i = 1; i <= daysInMonth(new Date(time).getMonth()); i++) {
            let dates: HTMLElement = document.createElement('div');
            dates.className = 'calendar__grid-item';

            dates.addEventListener('click', (event: Event) => {
                highlightDate(event.target);
            });

            calendarGrid.appendChild(dates).innerHTML = i.toString();
    ...
    function nextDates(length: number): void {
    ...
    function isWeekend(): void {
        for (let i = 0; i < 42; i++) {
            if (i % 7 == 0 + 5 || i % 7 == 0 + 6) {
                let weekend: NodeListOf<HTMLElement> = document.querySelectorAll('.calendar__grid-item')
                weekend[i].classList.add('calendar__grid-item--weekend')
            }
        }
    }

    function drawDates(): void {
        prevDates();
        mainDates();
    ...
    let isDateHighlighted: boolean = false;
    ...
    function highlightDate(target: EventTarget): void {
        let dates: NodeListOf<HTMLElement> = document.querySelectorAll('.calendar__grid-item');
        ...
        (target as HTMLElement).classList.add('calendar__grid-item--highlight');
    ...
    const backgroundItem1: HTMLElement = document.createElement('div');
    backgroundItem1.className = 'background__item-1';
    background.appendChild(backgroundItem1);

    const tasks: HTMLElement = document.createElement('div');
    tasks.className = 'tasks';
    background.appendChild(tasks);

    const tasksTitle: HTMLElement = document.createElement('p');
    tasksTitle.className = 'tasks__title';
    tasks.appendChild(tasksTitle).innerHTML = 'Your Tasks';

    const tasksAddTask: HTMLElement = document.createElement('div');
    tasksAddTask.className = 'tasks__plus-item';
    tasks.appendChild(tasksAddTask).innerHTML = '+';

    const backgroundItem2: HTMLElement = document.createElement('div');
    backgroundItem2.className = 'background__item-2';
    background.appendChild(backgroundItem2);

    const backgroundLine: HTMLElement = document.createElement('div');
    backgroundLine.className = 'background__line';
    background.appendChild(backgroundLine);

    const backgroundNameItem: HTMLElement = document.createElement('div');
    backgroundNameItem.className = 'background__name-item';
    background.appendChild(backgroundNameItem);

    const backgroundName: HTMLElement = document.createElement('div');
    backgroundName.className = 'background__name';
    backgroundNameItem.appendChild(backgroundName).innerHTML = 'Stanislav Chernov';

    const backgroundTitle: HTMLElement = document.createElement('div');
    backgroundTitle.className = 'background__title';
    background.appendChild(backgroundTitle).innerHTML = 'Calendar';

    //start btn
    const startBtn: HTMLElement = document.createElement('div');
    startBtn.className = 'start-btn';
    document.body.appendChild(startBtn).innerHTML = 'Start';

    const startBorder1: HTMLElement = document.createElement('span');
    startBorder1.className = 'start-btn__border-1';
    startBtn.appendChild(startBorder1);

    const startBorder2: HTMLElement = document.createElement('span');
    startBorder2.className = 'start-btn__border-2';
    startBtn.appendChild(startBorder2);

    const startBorder3: HTMLElement = document.createElement('span');
    startBorder3.className = 'start-btn__border-3';
    startBtn.appendChild(startBorder3);

    const startBorder4: HTMLElement = document.createElement('span');
    startBorder4.className = 'start-btn__border-4';
    ...
    function backgroundNameMouseOver(): void {
    ...
    function backgroundNameMouseOut(): void {
        backgroundNameItem.classList.toggle('background__name-item--hover');
        backgroundName.classList.toggle('background__name--hover');
    }

    function startBtnMouseOver(): void {
        startBtn.classList.toggle('start-btn--hover');
        startBorder1.classList.toggle('start-btn__border-1--hover');
        startBorder2.classList.toggle('start-btn__border-2--hover');
        startBorder3.classList.toggle('start-btn__border-3--hover');
        startBorder4.classList.toggle('start-btn__border-4--hover');
    ...
    function startBtnMouseOut(): void {
        startBtn.classList.toggle('start-btn--hover');
    }

    function startBtnMouseClick(): void {
    ...
    startBtn.addEventListener('click', () => {
        document.querySelector('.hello-dog').classList.add('hello-dog--hidden');
        setTimeout(() => {
            document.querySelector('.hello-dog').remove()
        }, 1000);
    });
